        ...
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            ...
            if (post.value === '' || post.value === 'select') {
              ...
              if (errMsg) {
                var _ref, _ref2, _post$placeholder;
                errMsg.innerHTML = "".concat((_ref = (_ref2 = (_post$placeholder = post.placeholder) !== null && _post$placeholder !== void 0 ? _post$placeholder : post.label) !== null && _ref2 !== void 0 ? _ref2 : post.id) !== null && _ref !== void 0 ? _ref : asterisk, " cannot be left empty");
    ...
    return _regeneratorRuntime().wrap(function _callee$(_context) {
      while (1) switch (_context.prev = _context.next) {
        case 0:
          ...
          reCaptchaToken = _args.length > 4 && _args[4] !== undefined ? _args[4] : null;
          ...
          form = (0,_global__WEBPACK_IMPORTED_MODULE_0__.id)(formId); // Add another input with ID 'token' and name 'token'
          tokenInput = document.createElement('input');
          tokenInput.type = 'hidden'; // Hidden input
          tokenInput.id = 'reCaptchaToken';
          tokenInput.name = 'reCaptchaToken';
          tokenInput.value = reCaptchaToken; // Replace 'YOUR_TOKEN_VALUE' with the actual token value
...
var LoginSubmission = function LoginSubmission(token) {
  e.preventDefault(); // Will also print the token

  // grecaptcha.ready(function() {
  //     grecaptcha.execute('MIX.RECAPTCHA_KEY', { action: 'login' }).then(function(token) {
  ...
  try {
    ...
    if (formData.error.length === 0) {
      ...
      // postFormData(loginURL, "loginNow", redirect, 'bulma', token);
    } else {
