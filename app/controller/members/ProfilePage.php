...
final class ProfilePage extends ProcessImg
{
    ...
    /**
     * Async endpoint for AI Biography Generation (GDPR Compliant)
     */
    public function getAIBiography(): void
    {
        try {
            \Src\CorsHandler::setHeaders();
            $targetId = isset($_GET['id']) ? checkInput($_GET['id']) : $this->id;
            
            // 1. Check if cached biography exists
            $db = self::connect2();
            $stmt = $db->prepare("SELECT biography FROM family_biographies WHERE user_id = ?");
            $stmt->execute([$targetId]);
            $cachedBio = $stmt->fetchColumn();
            
            if ($cachedBio) {
                msgSuccess(200, ['biography' => $cachedBio]);
                return;
            }

            // 2. Check GDPR AI Consent
            $stmt = $db->prepare("SELECT ai_consent FROM account WHERE id = ?");
            $stmt->execute([$targetId]);
            $consent = $stmt->fetchColumn();

            if (empty($consent) || (int)$consent !== 1) {
                // Return gracefully without AI processing
                msgSuccess(200, ['biography' => null, 'message' => 'AI consent not granted.']);
                return;
            }

            // 3. Fetch data for prompt
            $singleCust = new \App\model\SingleCustomerData();
            $memberInfo = $singleCust->getCustomerData((string)$targetId, ['personal']);
            $firstName = $memberInfo['firstName'] ?? 'Family Member';
            $lastName = $memberInfo['lastName'] ?? '';
            
            // Fetch last 10 public posts
            $posts = \App\model\Post::getAllPostProfilePics(); 
            // NOTE: getAllPostProfilePics gets feed posts, we should filter by targetId
            $userPosts = array_filter($posts, function($p) use ($targetId) {
                return (string)$p['id'] === (string)$targetId;
            });
            $userPosts = array_slice($userPosts, 0, 10);
            $postMessages = array_column($userPosts, 'postMessage');

            // 4. Call AI Service
            $biography = \App\services\AIService::generateBiography($firstName, $lastName, $postMessages);

            // 5. Cache result safely inside a transaction
            $db->beginTransaction();
            try {
                $insertStmt = $db->prepare("INSERT INTO family_biographies (user_id, biography) VALUES (?, ?)");
                $insertStmt->execute([$targetId, $biography]);
                $db->commit();
            } catch (\Exception $e) {
                $db->rollBack();
                error_log("Failed to save AI Bio: " . $e->getMessage());
            }

            msgSuccess(200, ['biography' => $biography]);

        } catch (\Throwable $th) {
            error_log((string)$th);
            msgException(500, "Unable to load AI Biography");
        }
    }
}
