...
{{ $data['firstName'] }} {{ $data['lastName'] }} sent you a request to join his family network <br><br>
        ...
        <tr>
          <td style="text-align: center;">
            <div style="padding: 0px; background-color: #f8f8f8; border-radius: 8px;">
              ...
              <img src={{ getenv("MIX_ROUTE_PICS") }}{{ $data['profileImg'] }} alt="Avatar" width="50" height="50" style="display: block; margin: 10px auto;">

              <p style="font-size: 18px; margin: 0;">{{ $data['firstName'] }} {{ $data['lastName'] }}</p>
              ...
              <div style="display: flex; justify-content: space-between; margin-top: 10px; margin-left: 70px margin-bottom: 10px;">
                ...
                <a href="{{ getenv('MIX_APP_URL2') }}member/request/{{ $data['id'] }}/{{ $data['approverId'] }}/50/{{ $data['famCode'] }}/email" style="text-decoration: none; background-color: #4CAF50; color: white; border: none; padding: 8px 16px; border-radius: 4px; margin-top: 10px;">
                 Accept
                </a>

                <a href="{{ getenv("MIX_APP_URL2") }}member/request/{{ $data['id'] }}/{{ $data['approverId'] }}/10/request/email"  
